const DATABASE_NAME = "haneoka-chart-editor";
const DATABASE_VERSION = 1;
const STORE_NAME = "drafts";
const ACTIVE_DRAFT = "active";

export interface StoredChartEditorDraft<Project> {
  project: Project;
  audio?: Blob;
  audioName?: string;
  updatedAt: number;
...
export const loadChartEditorDraft = async <Project>(): Promise<StoredChartEditorDraft<Project> | undefined> =>
  withStore("readonly", (store) => store.get(ACTIVE_DRAFT));

export const saveChartEditorDraft = async <Project>(draft: StoredChartEditorDraft<Project>): Promise<void> => {
  await withStore("readwrite", (store) => store.put(draft, ACTIVE_DRAFT));
};

export const clearChartEditorDraft = async (): Promise<void> => {
